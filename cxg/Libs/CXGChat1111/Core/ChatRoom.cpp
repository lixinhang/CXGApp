...
long CChatRoom::Speak(std::string words, std::string uid, bool ispublic) {
    ...
    if(uid == "0") {  //to all the room users; ispublic is to all
        ...
        if(ispublic) {  //
            ...
            jsonSpeaker += std::to_string(m_nRoomId);
            jsonSpeaker += "\",\"uid\":\"";
            jsonSpeaker += std::to_string(m_nMasterId);
            jsonSpeaker += "\",\"ct\":\"" ;
            jsonSpeaker += words;
...
void CChatRoom::SendChatMsg(std::string strMsg,std::string strMasterid, bool bPrivate)
{
    ...
    std::string jsonChatMsg = "{\"_method_\":\"SendPubMsg\",\"toMasterId\":\"";
    jsonChatMsg += strMasterid;
    jsonChatMsg += "\",\"toMasterNick\":\"\",\"rid\":\"";
    jsonChatMsg += m_nRoomId;
    jsonChatMsg += "\",\"uid\":\"";
    jsonChatMsg += m_nMasterId;
    jsonChatMsg += "\",\"ct\":\"" ;
    jsonChatMsg += strMsg;
    jsonChatMsg += "\",\"pub\":\"0\",\"key\":\"\",\"code\":\"\",\"checksum\":\"\",\"v\",\"0\"}";
