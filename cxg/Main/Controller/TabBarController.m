...
- (void)viewDidLoad
{
    ...
    _lastViewController = self.childViewControllers.lastObject;
...
- (void)addCameraButton
{
    ...
    [cameraBtn setImage:[UIImage imageNamed:@"tab_live"] forState:UIControlStateNormal];
    [cameraBtn setImage:[UIImage imageNamed:@"tab_live"] forState:UIControlStateHighlighted];
    
    // 自适应,自动根据按钮图片和文字计算按钮尺寸
    [cameraBtn sizeToFit];
    
    cameraBtn.center = CGPointMake(self.tabBar.xj_width * 0.5, self.tabBar.xj_height * 0.5 + 2);
...
{
    //设置TabBar按钮的内容
    RankListViewController *rankListVc = self.childViewControllers[0];
    rankListVc.tabBarItem.image = [UIImage imageNamed:@"tab_rank"];
    rankListVc.tabBarItem.selectedImage = [UIImage imageWithOriginalRenderingMode:@"tab_rank_select"];
    ...
    mineVc.tabBarItem.image = [UIImage imageNamed:@"tab_user"];
    mineVc.tabBarItem.selectedImage = [UIImage imageWithOriginalRenderingMode:@"tab_user_select"];
    
    // 调整TabBarItem位置
    UIEdgeInsets insets = UIEdgeInsetsMake(13, 0, -13, 0);
    //UIEdgeInsets cameraInsets = UIEdgeInsetsMake(-10, 0, 10, 0);
    
    rankListVc.tabBarItem.imageInsets = insets;
